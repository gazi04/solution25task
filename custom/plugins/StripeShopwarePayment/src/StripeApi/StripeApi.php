...
class StripeApi
{
    ...
    private function getSavedPaymentMethodsOfStripeCustomer(string $stripeCustomerId, string $paymentMethodType): array
    {
        ...
        try {
            ...
            $this->logger->error("Failed to get saved payment methods for $stripeCustomerId: " . $exception->getMessage());
            return [];
        }

        // Sort the payment methods by id (which correspond to the date, the payment method was created/added)
        usort($paymentMethods, fn ($paymentMethodA, $paymentMethodB) => $paymentMethodA->id <=> $paymentMethodB->id);
